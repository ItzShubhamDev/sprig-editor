export async function runGame({ api }) {
    const {
        setMap,
        addText,
        clearText,
...
88888888888888888888`;
    ...
    function prepareGroundMap() {
        ...
        selected == 0
            ? addSprite(0, 0, charmander)
            : addSprite(0, 0, bulbasaur);
    ...
    function addCounterTexts() {
        ...
        clearText();
        addText(`${collectedMons} / ${totalMons}`, {
            x: 4,
            y: 0,
            color: collectedMons == totalMons ? yellow : white,
        });
        addText(`${pokeballs} / ${totalBalls}`, {
            x: 12,
            y: 0,
            color: pokeballs == totalBalls ? yellow : white,
        });
    }
    ...
    function checkNearbyPokemon() {
        ...
        for (let i = x - 1; i <= x + 1; i++) {
            for (let j = y - 1; j <= y + 1; j++) {
                ...
                for (const sprite of sprites) {
                    if (
                        pokemons.includes(sprite.type) &&
                        sprite.type != pokemons[selected]
                    ) {
    ...
    function hpBars() {
        ...
        let hp1Percent = (participantHp / hp1) * 100;
        let hp2Percent = (opponentHp / hp2) * 100;
        ...
        if (state == "Atk") {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                addText(mv.name, {
                    x: 1,
                    y,
                    color: mv.name == selectedMove ? yellow : white,
                });
    ...
    function atk() {
        ...
        if (mv != "Pass") {
            ...
            p1HitCheck = checkAccuracyResult(
                parseInt(p1Move["accuracy"]) / 100
            );
        }
        const p2Moves = [];
        ...
        const p2HitCheck = checkAccuracyResult(
            parseInt(p2Move["accuracy"]) / 100
        );
        let p1Dmg = 0,
            p2Dmg = 0;
        let eff1 = 1,
        ...
        if (p1HitCheck) {
            ...
            if (p1Move["class"] == "physical") {
                p1Dmg = calculateDmg(
                    parseInt(p1["stats"]["attack"]),
                    parseInt(p2["stats"]["defense"]),
                    parseInt(p1Move["power"]),
                    eff1
                );
            } else {
                p1Dmg = calculateDmg(
                    parseInt(p1["stats"]["spAttack"]),
                    parseInt(p2["stats"]["spDefense"]),
                    parseInt(p1Move["power"]),
                    eff1
                );
            }
        ...
        if (p2HitCheck) {
            ...
            if (p2Move["class"] == "physical") {
                p2Dmg = calculateDmg(
                    parseInt(p2["stats"]["attack"]),
                    parseInt(p1["stats"]["defense"]),
                    parseInt(p2Move["power"]),
                    eff2
                );
            } else {
                p2Dmg = calculateDmg(
                    parseInt(p2["stats"]["spAttack"]),
                    parseInt(p1["stats"]["spDefense"]),
                    parseInt(p2Move["power"]),
                    eff2
                );
            }
        ...
        setMap(battleMap);
        selectedMap = battleMap;
        let p1MoveName = mv != "Pass" ? p1Move["name"] : "None";
        const pass = mv == "Pass";
        setMovesText(
            p1["name"],
            p2["name"],
...
*/
        try {
            if (
                typeChart[attackType][t1] == 0 ||
                (t2 && t2.length > 0 && typeChart[attackType][t2] == 0)
            )
                effectiveness = 0;
    ...
    function calculateDmg(atk, def, power, eff) {
        ...
        const atkDefDamage =
            (intitialDamage * power * powerModifier * atkDefModifier) / 50 +
            random(5, 10);

...
const normalizeStack = (stack: string): StackItem[] | null => {
    ...
    if (lines.every((line) => firefoxStackRegex.test(line))) {
        ...
        lines
            ...
            .every(
                (line) =>
                    chromeStackRegex.test(line) ||
                    chromeStackOnlyUrlRegex.test(line)
            )
    ) {
        lines.shift();
        return lines.map((line) => {
...
export const normalizeGameError = (gameError: GameError): NormalizedError => {
    ...
    if (gameError.kind === "parse") {
        ...
        const stack =
            (gameError.error.stack
                ? normalizeStack(gameError.error.stack)
                : null) ?? [];
        ...
        if (!addedLineCol && line && col && stack[0]) {
            descriptionLines.unshift(
                `    at ${stack[0]!.callSite} (index.ts:${line}:${col})`
            );
        ...
        descriptionLines.unshift(
            `${gameError.error.name}: ${gameError.error.message}`
        );
        return {
        ...
        return {
            description: `Runtime Error: ${gameError.error}`,
            raw: gameError.error,
        };
    }
};

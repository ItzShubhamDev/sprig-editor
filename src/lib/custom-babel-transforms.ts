export default function TransformDetectInfiniteLoop({
    types: t,
}: {
    types: any;
}) {
    const MAX_ITERATIONS = +import.meta.env.PUBLIC_MAX_ITERATIONS || 2000;
    const MAX_LOOP_TIME_MS = +import.meta.env.PUBLIC_MAX_LOOP_TIME_MS || 1500;
    return {
        visitor: {
            "WhileStatement|ForStatement|DoWhileStatement": (path: any) => {
                ...
                const countReInitializer = t.assignmentExpression(
                    "=",
                ...
                const timeReInitializer = t.assignmentExpression(
                    "=",
                ...
                if (!path.get("body").isBlockStatement()) {
                    ...
                    path.get("body").replaceWith(
                        t.blockStatement([guard, statement])
                    );
                } else {
                ...
                FunctionDeclaration: (path: any) => {
                    ...
                    if (engineApiKeys.includes(functionName)) {
                        ...
                        throw new TransformError(
                            `Cannot redeclare built-in function: ${functionName} (${loc.line}:${loc.column})`,
                            loc
                        );
            ...
            StringLiteral(path: any) {
                ...
                if (value.includes("`")) {
                    ...
                    throw path.buildCodeFrameError(
                        `Backtick found within ${
                            quoteType === '"' ? "double" : "single"
                        }-quoted string at (${loc.line}:${loc.column})`
                    );
                }
            },
        },
